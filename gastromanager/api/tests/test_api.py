...
def test_create_recipe(create_ingredient):
    ...
    RecipeIngredient.objects.create(recipe_name=recipe, ingredient=ingredient, quantity=100)
    return recipe
...
def test_create_ice_cream_production(create_recipe, create_user):
    recipe = create_recipe
    user = create_user
    return IceCreamProduction.objects.create(recipe=recipe, container_size=0.5, quantity_produced=10, produced_by=user)

@pytest.fixture
def test_create_ice_cream_stock_take_out(create_stock_item, create_user):
    stock_item = create_stock_item
    user = create_user
    return IceCreamStockTakeOut.objects.create(ice_cream_production=stock_item, quantity_moved=10, moved_by=user)
...
def test_ice_cream_production_and_inventory_update(create_recipe, create_user):
    recipe = create_recipe()
    user = create_user()
    production = IceCreamProduction.objects.create(recipe=recipe, container_size=0.5, quantity_produced=10, produced_by=user)
    inventory = IngredientInventory.objects.get(ingredient_name=recipe.recipeingredient_set.first().ingredient)
    stock_item = StockItem.objects.get(recipe=recipe, size=0.5)

    assert inventory.quantity == 100 - (recipe.recipeingredient_set.first().quantity * production.quantity_produced)
...
def test_negative_production(create_recipe, create_user):
    ...
    with pytest.raises(ValidationError):
        IceCreamProduction.objects.create(recipe=recipe, container_size=0.5, quantity_produced=200, produced_by=user)

# Update Stock Test: Check that stock updates correctly when adding production to existing stock.
@pytest.mark.django_db
def test_update_stock_on_production_existing_stock_item(create_recipe, create_user, client):
    recipe = create_recipe()
    user = create_user()
...
def test_stock_takeout(create_stock_item, create_user):
    ...
    user = create_user()
    quantity_taken = 10
    stock_takeout = IceCreamStockTakeOut.objects.create(ice_cream_production=stock_item, quantity_moved=quantity_taken, moved_by=user)
...
def test_negative_stock_takeout(create_stock_item, create_user):
    ...
    user = create_user()
    with pytest.raises(ValidationError):
        IceCreamStockTakeOut.objects.create(ice_cream_production=stock_item, quantity_moved=200, moved_by=user)
...
def test_stock_takeout_not_enough_stock(create_stock_item, create_user):
    stock_item = create_stock_item()
    user = create_user()
    new_stock_item = StockItem.objects.create(recipe=stock_item.recipe, size=0.5, quantity=5)  # Low stock

    with pytest.raises(ValidationError):
        IceCreamStockTakeOut.objects.create(ice_cream_production=new_stock_item, quantity_moved=10, moved_by=user)
...
def test_edit_recipe_with_ingredients(create_recipe, client):
    ...
    assert Recipe.objects.get(pk=recipe.id).flavor == 'Updated Flavor'  # Ensure the recipe is updated
    assert recipe.recipeingredient_set.count() == recipe_ingredient_count  # The ingredient count should not change
...
def test_create_recipe_with_ingredients_and_quantities(create_ingredient, client):
    ...
    assert new_recipe.recipeingredient_set.get(ingredient=ingredient1).quantity == 100  # Verify ingredient quantities
...
def test_stock_update_on_adding_new_recipe(create_recipe, create_user, client):
    ...
    production = IceCreamProduction.objects.create(recipe=new_recipe, container_size=0.5, quantity_produced=5, produced_by=user)
    stock_item.refresh_from_db()
...
@pytest.mark.django_db
def test_inventory_update_on_recipe_ingredient_deletion(create_recipe, create_ingredient, client):
    recipe = create_recipe()
    ingredient = create_ingredient()
    ...
    response = client.post(reverse('update_recipe', args=[recipe.id]), {'recipeingredients_set-TOTAL_FORMS': 0, 'recipeingredients_set-INITIAL_FORMS': 1})
    updated_inventory_quantity = IngredientInventory.objects.get(ingredient_name=ingredient).quantity

    assert initial_inventory_quantity == updated_inventory_quantity + 50  # Inventory should decrease correctly
...
def test_inventory_update_on_ingredient_addition(create_ingredient, client):
    ...
    updated_inventory_quantity = IngredientInventory.objects.get(ingredient_name__name='New Ingredient').quantity

    assert initial_inventory_quantity + 100 == updated_inventory_quantity  # Inventory should increase correctly
...
def test_add_ingredient_with_expiration_date(create_ingredient, client):
    ...
    response = client.post(reverse('add_ingredient'), data)
    updated_inventory_quantity = IngredientInventory.objects.get(ingredient_name__name='New Ingredient with Expiration Date').quantity

    assert initial_inventory_quantity + 200 == updated_inventory_quantity  # Inventory should increase correctly
    assert IngredientIncoming.objects.filter(ingredient__name='New Ingredient with Expiration Date').count() == 1  # There should be one ingredient incoming record
...
def test_insufficient_stock_takeout(create_stock_item, create_user, client):
    stock_item = create_stock_item()
    user = create_user()
    quantity_taken = stock_item.quantity + 1  # Attempt to take more than what is available
    with pytest.raises(ValidationError):
        response = client.post(reverse('stock_takeout'), {'stock_item': stock_item.id, 'quantity_moved': quantity_taken})
